...
class VoxelVRApp:
    ...
    def _preview_loop(self) -> None:
        ...
        ui_update_interval = 0.033  # Update UI at ~30 FPS
        ...
        while self.preview_active and self.state.is_running:
            ...
            is_calibrating = (self.calibration_panel.state.is_running and 
               self.calibration_panel.state.current_step == CalibrationStep.CALIBRATION)
            
            # Only run detection periodically during calibration
            if is_calibrating and (now - last_detection_time) >= detection_interval:
            ...
            # Throttle UI updates for non-calibration mode
            elif (now - last_ui_update_time) >= ui_update_interval:
                last_ui_update_time = now
                # Not calibrating or between detection intervals, just show raw frames
                for cam_id, frame in frames.items():
                    self.update_camera_frame(cam_id, frame.image)
            
            # Small sleep to prevent CPU spinning, but don't block too long
            time.sleep(0.001)
    ...
    def _update_calibration_progress(self) -> None:
        ...
        if dpg.does_item_exist("calib_progress_container"):
            ...
            # Add pairwise progress grid
            dpg.add_separator(parent="calib_progress_container")
            dpg.add_text("Pairwise Calibration:", color=(150, 150, 150), parent="calib_progress_container")
            
            pairwise = progress.get('pairwise', {})
            is_connected = progress.get('pairwise_connected', False)
            disconnected = progress.get('pairwise_disconnected_cameras', [])
            
            if pairwise:
                camera_ids = sorted(set(c for pair in pairwise.keys() for c in pair))
                
                # Build header row
                with dpg.group(horizontal=True, parent="calib_progress_container"):
                    dpg.add_text("      ", tag="pairwise_header_spacer")
                    for cam_id in camera_ids[1:]:
                        dpg.add_text(f" {cam_id:2d} ", tag=f"pairwise_header_{cam_id}")
                
                # Build grid rows
                for i, cam_a in enumerate(camera_ids[:-1]):
                    with dpg.group(horizontal=True, parent="calib_progress_container"):
                        dpg.add_text(f"Cam {cam_a}:", tag=f"pairwise_row_{cam_a}")
                        
                        for cam_b in camera_ids[i+1:]:
                            pair_key = (cam_a, cam_b)
                            pair_data = pairwise.get(pair_key, {'percent': 0, 'frames': 0, 'required': 10, 'ready': False})
                            
                            # Determine color based on progress
                            if pair_data['ready']:
                                color = (100, 200, 100)  # Green
                                icon = "●"
                            elif pair_data['frames'] > 0:
                            ...
                            tooltip = f"Cam {cam_a} ↔ Cam {cam_b}: {pair_data['frames']}/{pair_data['required']}"
                            text_tag = f"pairwise_cell_{cam_a}_{cam_b}"
                            dpg.add_text(f" {icon} ", color=color, tag=text_tag)
                            
                            with dpg.tooltip(text_tag):
                                dpg.add_text(tooltip)
                
                # Connectivity status
                dpg.add_spacer(height=5, parent="calib_progress_container")
                if is_connected:
                    dpg.add_text("✓ All cameras connected!", color=(100, 200, 100), parent="calib_progress_container")
                elif disconnected:
                    dpg.add_text(f"⚠ Cameras not connected: {disconnected}", color=(200, 150, 100), parent="calib_progress_container")
                else:
                    dpg.add_text("Show board to camera pairs to calibrate", color=(150, 150, 150), parent="calib_progress_container")
